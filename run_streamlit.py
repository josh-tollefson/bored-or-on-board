...
def run_sentiment_model(df, modelpath, modelfile, vectorpath, vectorfile):
	'''Does the main modeling work for comment sentiment'''
...
def run_label_model(df, modelpath, modelfile, vectorpath, vectorfile, top_n=10):
	'''Does the main modeling work for comment categorization 
	by selecting most unique and highly weighted word features'''
...
def file_selector():
	uploaded_file = st.file_uploader("Choose a CSV file containing playtester comments in one column, one comment per row (default is Monopoly example)", type="csv")
	if uploaded_file is None:
		return './examples/monopoly_comments.csv'
	return uploaded_file #os.path.join(folder_path, selected_filename)
